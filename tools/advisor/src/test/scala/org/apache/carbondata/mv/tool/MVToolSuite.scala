...
class MVToolSuite extends TestHiveSingleton {
  class MVToolTest extends MVToolBase {
    override protected def getMVFilePath: String = "D:/mvout/mv-candidate.sql"
...
//    sql("CREATE TABLE maintable_par(id int, name string, city string, age string) STORED AS PARQUET")
//    sql(s"Insert into table maintable_par select * from maintable")
    val table2columnset1 = Map("maintable_par" -> Set("id", "name", "city", "age"))
    ...
    for (tableName <- table2columnset1.keySet) {
      val sqlDF = sql(s"SELECT * FROM $tableName LIMIT 3")
//      sqlDF.show()
      sql(s"ANALYZE TABLE $tableName COMPUTE STATISTICS").show(1000)
      val cols = table2columnset1.get(tableName).map(_.mkString(", ")).getOrElse("")
      sql(s"ANALYZE TABLE $tableName COMPUTE STATISTICS FOR COLUMNS $cols").show(1000)
    }

    val queryLogEntryPattern = """^[{]\"sql\":\s*\"(.*?)\"[}]$"""
    val mvtool = new MVToolTest
    
    mvtool.adviseMVs(sqlContext.sparkSession, TestQueryBatchPreprocessor, new TestCommonSubexpressionManager(sqlContext.sparkSession, sqlContext.sparkSession.sessionState.conf), "D:/samplesql.sql", queryLogEntryPattern)
  }
}