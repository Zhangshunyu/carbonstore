...
//TODO: generalize this using SQLBuild to handle nested sub-query expression, using the correspondence
...
class SQLBuilder private (
  ...
  def this(modularPlan: ModularPlan) = 
    this(SQLBuilder.subqueryPreprocess(modularPlan, new AtomicLong()), new AtomicLong(0), "gen_subquery_")
  ...
  private def newSubqueryName(): String = s"${subqueryPrefix}${nextSubqueryId.getAndIncrement()}"
  ...
  object SQLizer extends RuleExecutor[ModularPlan] {
    override protected def batches: Seq[Batch] = Seq(
        Batch("Recover Scoping Info", Once,
            // Clean up qualifiers due to erasure of sub-query aliases to use base tables as qualifiers
            ...
            // Comment out for now, will add later by converting AttributMap to Map in SQLBuildDSL.scala 
//            RemoveCasts
            )
        )
    object CleanupQualifier extends Rule[ModularPlan] {
      override def apply(tree: ModularPlan): ModularPlan = tree transformDown {
        case s @ modular.Select(_, _, _, _, _, Seq(g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _)), _, _, _) if (!s.rewritten && s2.children.forall { _.isInstanceOf[modular.LeafNode] }) =>
          val attrMap = AttributeMap(s2.outputList.collect { case a: Alias if (a.child.isInstanceOf[Attribute]) => (a.toAttribute, a) })
          cleanupQualifier(s, s2.aliasMap, s2.children, attrMap)

        case g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _) if (!g.rewritten && s2.children.forall { _.isInstanceOf[modular.LeafNode] }) =>
          val attrMap = AttributeMap(s2.outputList.collect { case a: Alias if (a.child.isInstanceOf[Attribute]) => (a.toAttribute, a) })
          cleanupQualifier(g, s2.aliasMap, s2.children, attrMap)

        case s @ modular.Select(_, _, _, _, _, _, _, _, _) if (!s.rewritten && s.children.forall { _.isInstanceOf[modular.LeafNode] }) => {
          cleanupQualifier(s, s.aliasMap, s.children, AttributeMap(Seq.empty[(Attribute, Alias)]))
        }
      }

      private def cleanupQualifier(node: ModularPlan, aliasMap: Map[Int, String], children: Seq[ModularPlan], attrMap: AttributeMap[Alias]): ModularPlan = node transform {
          ...
          plan.transformExpressions {
            case ref: Attribute =>
              ...
              if (i > -1) { 
                ...
                if (aliasMap.nonEmpty && aliasMap.get(i).nonEmpty) AttributeReference(ref.name, ref.dataType)(exprId = ref.exprId, qualifier = Option(aliasMap(i)))
                else ref
              }
              else {
    ...
    object AddSubquery extends Rule[ModularPlan] {
      override def apply(tree: ModularPlan): ModularPlan = tree transformUp {
        case s @ modular.Select(_, _, _, _, _, Seq(g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _)), _, _, _) if (s2.children.forall { _.isInstanceOf[modular.LeafNode] }) =>
          s
        case g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _) if (s2.children.forall { _.isInstanceOf[modular.LeafNode] }) =>
          g
        case s @ modular.Select(_, _, _, _, _, _, _, _, _) if (!s.rewritten && !s.children.forall { _.isInstanceOf[modular.LeafNode] }) => {
          ...
          s.children.zipWithIndex.filterNot { _._1.isInstanceOf[modular.LeafNode] }.foreach {
            case (child: ModularPlan, index) => { //if (!s.aliasMap.contains(index)) => {
              ...
              val windowAttributeSet = if (child.isInstanceOf[modular.Select]) {
                val windowExprs = child.asInstanceOf[modular.Select].windowSpec.map{case Seq(expr) => expr.asInstanceOf[Seq[NamedExpression]]}.foldLeft(Seq.empty.asInstanceOf[Seq[NamedExpression]])(_ ++ _)
...
//              TODO: how to use alias to avoid duplicate names with distinct exprIds
//              val duplicateNames = collectDuplicateNames(subqueryAttributeSet).toSet
              if (SQLBuilder.collectDuplicateNames(subqueryAttributeSet).nonEmpty) 
                new UnsupportedOperationException(s"duplicate name(s): ${child.output.map(_.toString + ", ")}")
              list = list :+ ((index, subqueryName))
              newS = newS.transformExpressions {
                case ref: Attribute if (subqueryAttributeSet.contains(ref)) =>
                  AttributeReference(ref.name, ref.dataType)(exprId = ref.exprId, qualifier = Some(subqueryName))
                case alias: Alias if (subqueryAttributeSet.contains(alias.toAttribute)) =>
                  Alias(alias.child, alias.name)(exprId = alias.exprId, qualifier = Some(subqueryName))
              }
            }
            case _ => 
          }
          if (!list.isEmpty) {
            list = s.aliasMap.toList ++ list
        ...
        case g @ modular.GroupBy(_, _, _, _, _, _, _) if (!g.rewritten && g.alias.isEmpty) => {
          ...
          if (SQLBuilder.collectDuplicateNames(subqueryAttributeSet).nonEmpty) 
            new UnsupportedOperationException(s"duplicate name(s): ${newG.child.output.map(_.toString + ", ")}")
  ...
  object RemoveCasts extends Rule[ModularPlan] {
      def apply(tree: ModularPlan): ModularPlan = tree transformAllExpressions {
...
object SQLBuilder {
    def collectAttributeSet(outputList: Seq[Expression]): AttributeSet = AttributeSet( outputList.collect {
      case a @ Alias(_,_) => a.toAttribute
      case a : Attribute => a
    }) 
    
    def collectDuplicateNames(s: AttributeSet) = {
    ...
    def subqueryPreprocess(plan: ModularPlan, nextScalarSubqueryId: AtomicLong): ModularPlan = {
      def newSubqueryName(nextScalarSubqueryId: AtomicLong): String = s"gen_expression_${nextScalarSubqueryId.getAndIncrement()}_"
      plan.transformAllExpressions {
        case s: ModularSubquery =>
          ...
          } else throw new UnsupportedOperationException(s"Expression $s can't converted to SQL")          
        case o => o
      }
    }
