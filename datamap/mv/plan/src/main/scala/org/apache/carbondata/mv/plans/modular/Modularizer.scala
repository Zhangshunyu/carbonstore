    ...
    val replaced = plan.transformAllExpressions {
      case s: ScalarSubquery =>
        if (s.children.isEmpty) {
          ScalarModularSubquery(modularize(s.plan).next.asInstanceOf[ModularPlan], s.children, s.exprId)
        }
        else throw new UnsupportedOperationException(s"Expression $s doesn't canonicalized")
      case l: ListQuery =>
        if (l.children.isEmpty) {
          ModularListQuery(modularize(l.plan).next.asInstanceOf[ModularPlan], l.children, l.exprId)
        }
        else throw new UnsupportedOperationException(s"Expression $l doesn't canonicalized")  
      case e: Exists =>
        if (e.children.isEmpty) {
          ModularExists(modularize(e.plan).next.asInstanceOf[ModularPlan], e.children, e.exprId)
        }
        else throw new UnsupportedOperationException(s"Expression $e doesn't canonicalized")        
      case o => o
    }
//    val replaced = plan
  ...
  private def modularizeCore(plan: LogicalPlan): Iterator[TreeType] = {
    ...
    supports(pruned.hasNext, s"Modular plan not supported (e.g. has subquery expression) for \n$plan")
