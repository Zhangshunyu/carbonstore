...
import org.apache.spark.sql.catalyst.expressions.{AttributeReference, AttributeSet, GroupingID, Cast, Grouping}
import org.apache.spark.sql.catalyst.expressions.{BitwiseAnd, ShiftRight, Literal}
import org.apache.spark.sql.types.{IntegerType, ByteType}
...
trait SQLBuildDSL {
  ...
  case class SPJGFragment(
      select: Seq[NamedExpression] = Nil, //TODO: more general, change to Seq[Either[Fragment,NamedExpression]]
    ...
    override def isUnsupported: Boolean = select == Nil || from == Nil || from.map(_._1.isUnsupported).foldLeft(false)(_ || _)
  ...
  //
  //  object SPJGFragment {
  //    type ReturnType = (Seq[NamedExpression],Seq[(Fragment, Option[JoinType],Seq[Expression])],Seq[Expression],Seq[Expression],Seq[Expression],Option[String])
  //      
  ...
  case class UNIONFragment(
      ...
      alias: Option[String] = None,
      modifiers: (FlagSet, Seq[Seq[Any]])) extends Fragment {
    override def isUnsupported: Boolean = union == Nil || union.map(_.isUnsupported).foldLeft(false)(_ || _)
    override def Supported: Boolean = !isUnsupported
  }
  ...
  def fragmentExtract(plan: ModularPlan, alias: Option[String]): Fragment = 
    if (plan.rewritten) fragmentExtract_Rewritten(plan, alias) 
    else fragmentExtract_NonRewritten(plan, alias)
 
  // Rewritten portion of query plan
  private def fragmentExtract_Rewritten(plan: ModularPlan, alias: Option[String]): Fragment = plan match {
    case s1 @ modular.Select(_, _, _, _, _, Seq(g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _)), _, _, _) //=> {
         if (!s1.skip && !g.skip && !s2.skip ) => extractRewrittenOrNonRewrittenSelectGroupBySelect(s1, g, s2, alias)
         
    case s1 @ modular.Select(_, _, _, _, _, Seq(g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _)), _, _, _) //=> {
         if (s1.skip && g.skip && s2.skip ) => extractRewrittenOrNonRewrittenSelectGroupBySelect(s1, g, s2, alias)
         
    case s1 @ modular.Select(_, _, _, _, _, Seq(g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _)), _, _, _) //=> {
         if (!s1.skip && !g.skip && s2.skip ) => extractRewrittenSelectGroupBy(s1, g, alias)
         
    case s1 @ modular.Select(_, _, _, _, _, Seq(s2 @ modular.Select(_, _, _, _, _, _, _, _, _)), _, _, _) //=> {
         if (!s1.skip && s2.skip ) => extractRewrittenSelect(s1, alias)
         
    case other => extractSimpleOperator(other, alias)
  }

  // Non-rewritten portion of query plan
  private def fragmentExtract_NonRewritten(plan: ModularPlan, alias: Option[String]): Fragment = plan match {
    case s1 @ modular.Select(_, _, _, _, _, Seq(g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _)), _, _, _) //=> {
         if (s1.aliasMap.isEmpty && !g.rewritten) => extractRewrittenOrNonRewrittenSelectGroupBySelect(s1, g, s2, alias)

    case g @ modular.GroupBy(_, _, _, _, s2 @ modular.Select(_, _, _, _, _, _, _, _, _), _, _) if (g.alias.isEmpty && !s2.rewritten) => {
      val fragmentList = s2.children.zipWithIndex.map { case (child, index) => fragmentExtract(child, s2.aliasMap.get(index)) }
      val fList = s2.joinEdges.map {
        e =>
          {
            (e.right, (fragmentList(e.right), Some(e.joinType), s2.extractRightEvaluableConditions(s2.children(e.left), s2.children(e.right))))
          }
      }.toMap
      val from = (0 to fragmentList.length - 1).map(index => fList.get(index).getOrElse((fragmentList(index), None, Nil)))
  ...
  private def extractRewrittenOrNonRewrittenSelectGroupBySelect(s1: modular.Select, g: modular.GroupBy, s2: modular.Select, alias: Option[String]): Fragment = {
    val fragmentList = s2.children.zipWithIndex.map { case (child, index) => fragmentExtract(child, s2.aliasMap.get(index)) }
    val fList = s2.joinEdges.map {
      e =>
        {
    ...
    val windowExprs = s1.windowSpec.map { case Seq(expr) => expr.asInstanceOf[Seq[NamedExpression]] }.foldLeft(Seq.empty.asInstanceOf[Seq[NamedExpression]])(_ ++ _)
    val having = s1.predicateList.map { case attr: Attribute => aliasMap.get(attr).map(_.child).getOrElse(attr); case expr: Expression => expr.transform { case a: Alias => a.child }; case other => other }
    ...
    val select = if (groupingSet.nonEmpty) {
      if (gSet.equals(sSet) && windowExprs.isEmpty) select_g
  ...
  private def extractRewrittenSelectGroupBy(s1: modular.Select, g: modular.GroupBy, alias: Option[String]): Fragment = {
    ...
    // TODO:  how to handle alias of attribute in MV
    val select = s1.outputList.map { attr => aliasMap.get(attr.toAttribute).getOrElse(attr) } ++ windowExprs
  ...
  private def extractSimpleOperator(operator: ModularPlan, alias: Option[String]): Fragment = operator match {
    case s @ modular.Select(_, _, _, _, _, _, _, _, _) => {
      val fragmentList = s.children.zipWithIndex.map { case (child, index) => fragmentExtract(child, s.aliasMap.get(index)) }
      val fList = s.joinEdges.map {
        e =>
          {
            (e.right, (fragmentList(e.right), Some(e.joinType), s.extractRightEvaluableConditions(s.children(e.left), s.children(e.right))))
          }
      }.toMap
      val from = (0 to fragmentList.length - 1).map(index => fList.get(index).getOrElse((fragmentList(index), None, Nil)))
      val excludesPredicate = from.flatMap(_._3).toSet
      val windowExprs = s.windowSpec.map { case Seq(expr) => expr.asInstanceOf[Seq[NamedExpression]] }.foldLeft(Seq.empty.asInstanceOf[Seq[NamedExpression]])(_ ++ _)
    ...
    case u @ modular.Union(_, _, _) => {
      UNIONFragment(
        u.children.zipWithIndex.map { case (child, index) => fragmentExtract(child, None) }, alias, (u.flags, u.flagSpec))
    }
    case d @ modular.ModularRelation(_, _, _, _, _) => {
      ...
      else TABLEFragment(Seq((d.output).toString()), alias, (d.flags, d.rest))
    }
  ...
  private def addGroupingSetIfNeeded(g: modular.GroupBy, s: modular.Select) = {    
    if (g.flags.hasFlag(EXPAND)) {
      ...
      val flagsNeedExprs = for {flag <- pickledListOrder 
                                if (g.flags.hasFlag(flag))} yield (flag)
    ...
    else (g.outputList, (g.predicateList, Seq.empty))
  }   
}
