...
import org.apache.carbondata.mv.plans.{ Pattern }
...
//    plans.filter(_.find { case n if n.subqueries.nonEmpty => true }.isEmpty)
  ...
  override protected def makeupAliasMappings(plans: Iterator[ModularPlan]): Iterator[ModularPlan] = {
    def makeup(plan: ModularPlan): ModularPlan = plan transform {
...
abstract class ModularPatterns extends Modularizer[ModularPlan] {
  ...
  object SelectModule extends Pattern with PredicateHelper {
    ...
    private[this] def makeSelectModule(
      ...
      Seq(modular.Select(output, input, predicate, aliasmap, joinedge, children, flags, flagSpec, windowSpec))
    }

    def apply(plan: LogicalPlan): Seq[ModularPlan] = plan match {
      case Distinct(ExtractSelectModule(output, input, predicate, aliasmap, joinedge, children, flags1, fspec1, wspec)) => {
        val flags = flags1.setFlag(DISTINCT)
        makeSelectModule(output, input, predicate, aliasmap, joinedge, flags, children.map(modularizeLater), fspec1, wspec)
      }
      case Limit(limitExpr, Distinct(ExtractSelectModule(output, input, predicate, aliasmap, joinedge, children, flags1, fspec1, wspec))) => {
        val flags = flags1.setFlag(DISTINCT).setFlag(LIMIT)
        makeSelectModule(output, input, predicate, aliasmap, joinedge, flags, children.map(modularizeLater), Seq(Seq(limitExpr))++fspec1, wspec)
      }
      case Limit(limitExpr, ExtractSelectModule(output, input, predicate, aliasmap, joinedge, children, flags1, fspec1, wspec)) => {
        val flags = flags1.setFlag(LIMIT)
        makeSelectModule(output, input, predicate, aliasmap, joinedge, flags, children.map(modularizeLater), Seq(Seq(limitExpr))++fspec1, wspec)        
      }
      case ExtractSelectModule(output, input, predicate, aliasmap, joinedge, children, flags1, fspec1, wspec) => {
        makeSelectModule(output, input, predicate, aliasmap, joinedge, flags1, children.map(modularizeLater), fspec1, wspec)
      }
      case Window(exprs,_, _, ExtractSelectModuleForWindow(output, input, predicate, aliasmap, joinedge, children, flags1, fspec1, wspec)) => {       
        val sel1 = makeSelectModule(output, input, predicate, aliasmap, joinedge, flags1, children.map(modularizeLater), fspec1, wspec)
        makeSelectModule(output.map(_.toAttribute), output.map(_.toAttribute), Seq.empty, Map.empty, Seq.empty, NoFlags, sel1, Seq.empty, Seq(Seq(exprs))++wspec)        
      }
      case _ => Nil
    }
  }
  ...
  object GroupByModule extends Pattern with PredicateHelper {
    ...
    private[this] def makeGroupByModule(
      ...
      child: ModularPlan,
      fspec: Seq[Seq[Any]]) = {
      val groupby = Some(modular.GroupBy(output, input, predicate, alias, child, flags, fspec))
      groupby.map(Seq(_)).getOrElse(Nil)
    }

    def apply(plan: LogicalPlan): Seq[ModularPlan] = plan match {
      case Limit(limitExpr, ExtractGroupByModule(output, input, predicate, alias, child, flags1, fspec1)) => {
        val flags = flags1.setFlag(LIMIT)
        makeGroupByModule(output, input, predicate, flags, alias, modularizeLater(child), Seq(Seq(limitExpr))++fspec1)        
      }
